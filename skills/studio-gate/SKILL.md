...
1. `python3 <studio>/studio.py check <gate>`. Fix what it reports, or ask the owning role to fix it. Use `gate raise --force` only when Tim has asked to see the work as it stands, and say so in the pack.
2. `python3 <studio>/studio.py gate raise <gate>` writes the pack scaffold to `gates/<gate>.md`.
3. Fill the pack. It is read on a phone, so keep everything before the links to one screen:
